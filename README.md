<div align="center">

# Ignacio Herrera Mejias
**Analista Programador | Especialista en Clean Architecture y Modelamiento de Datos**

---

### Sobre mi
Soy un profesional apasionado por el desarrollo de software, con un enfoque metodico en el **pensamiento logico** e **ingenieria de software**. Mi enfoque principal es crear soluciones escalables utilizando **Arquitecturas Limpias**.

**Analista Programador** en IP Chile (2024 - 2026)
Enfocado en el aprendizaje continuo y eficiencia tecnica.

---

### Stack Tecnologico

<p align="center"><strong>Lenguajes de Programacion</strong>strong></p>p>
<p align="center">
  <img src="https://skillicons.dev/icons?i=java,python,js,cs,php,ts" />
</p>p>

<br />

<p align="center"><strong>Frameworks y Librerias</strong>strong></p>p>
<p align="center">
  <img src="https://skillicons.dev/icons?i=react,angular,django,dotnet" />
</p>p>

<br />

<p align="center"><strong>Bases de Datos</strong>strong></p>p>
<p align="center">
  <img src="https://skillicons.dev/icons?i=mysql,mongodb,sqlserver" />
</p>p>

<br />

<p align="center"><strong>Herramientas y Otros</strong>strong></p>p>
<p align="center">
  <img src="https://skillicons.dev/icons?i=git,docker,vscode,html,css" />
</p>p>

---

### Proyectos Destacados

**[Proyecto Escolar](https://github.com/ignaciohdev/proyecto-escolar)**
Sistema de gestion escolar integral.
`Angular` . `Django` . `MongoDB`

**[CV Web](https://ignaciohdev.github.io/cv-web/)**
Portafolio profesional interactivo.
`HTML` . `CSS` . `JS`

---

### Contacto
<a href="mailto:Ignacio.AHM@outlook.com"><img src="https://skillicons.dev/icons?i=gmail" height="35" /></a>
<a href="https://linkedin.com/in/ignacioherreramejias"><img src="https://skillicons.dev/icons?i=linkedin" height="35" /></a>

</div></strong></strong>
